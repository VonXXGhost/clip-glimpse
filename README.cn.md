...
双击 `clip_glimpse.exe`（需在 [配置文件](#配置) 中设置 `default_mode`），
或使用子命令：
...
每个二维码携带一个结构化的二进制数据块（12 字节头）：

```
┌────────┬──────────┬─────────┬─────────┬──────────┬──────────────────┐
│  MAGIC │ VERSION  │  FLAGS  │   SEQ   │  TOTAL   │      CRC32      │
│  2 字节 │  1 字节  │  1 字节  │  2 字节  │  2 字节   │      4 字节      │
...
- 所有分块格式统一 —— 不再区分 SOS/DATA/EOS
- 每个分块携带完整原始文本的 CRC32
- 读取端**可从任意分块开始接收**（循环消费，O(n)）
- 收集齐 `total` 个不重复分块后：拼接 → lz4 解压 → CRC32 校验 → 交付
- 单条消息最多 100 个分块
...
通过 RGB 通道复用实现 3× 带宽提升：

- **原理**：每帧显示画面承载 **3 个独立 B&W QR 码**，分别以 R/G/B 通道编码。读取端独立提取每个通道并分别解码。
- **颜色方案**：每通道仅使用极值（0 或 255）→ 8 种纯色（RGB 立方体顶点），对屏幕压缩和拍摄极其鲁棒。
- **生成**：`generate_color_qr()` 对 1~3 个数据块分别调用 `QrCode::with_version`，逐模块合成（Dark→0, Light→255）。空通道填充白色。
- **解码**：`extract_channel_from_bgra()` 从 BGRA 捕获中提取 R/G/B 为单通道灰度数据，经**对比度拉伸** (`stretch_contrast`) 恢复完整动态范围后，分别送入 `rxing` 独立解码。
- **向后兼容**：B&W 模式为默认。可通过生成界面的 "Color (3x)" 复选框或配置中的 `color_mode = true` 启用。

| 模式 | 每帧负载 | V25-M 显示尺寸 |
|------|----------|---------------|
| B&W | 767 B | ~351×351 |
| **彩色** | **2301 B** (3 × 767) | **~351×351**（不变） |
...
主要配置项：
- `default_mode` — 不指定子命令时的默认模式：`"generate"` 或 `"read"`（可选）
...
- `color_mode` — 启用彩色 QR 模式，带宽提升 3 倍（默认 false）
